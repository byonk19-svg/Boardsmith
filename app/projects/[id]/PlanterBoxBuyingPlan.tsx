import type { PlanterBoxStockBoardMaterialGroup, PlanterBoxStockBoardViewModel } from "@/lib/plans/planter-box-stock-board-view-model";

export function PlanterBoxBuyingPlan({ viewModel, compact = false }: { viewModel: PlanterBoxStockBoardViewModel; compact?: boolean }) {
      ...
      {viewModel.materialGroups.length > 0 ? (
        <div className={`mt-4 grid gap-3 ${compact ? "" : "lg:grid-cols-2 print:grid-cols-2"}`}>
          {viewModel.materialGroups.map((group) => (
            <PlanterBuyingMaterialGroup key={group.id} group={group} compact={compact} />
          ))}
        </div>
      ) : (
        <p className="mt-4 rounded-md border border-sawdust bg-shop/40 p-3 text-sm leading-6 text-ink/65">No modeled planter material groups are available yet.</p>
      )}

      {viewModel.reviewReasons.length > 0 ? (
        <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-3">
          <p className="text-sm font-semibold text-amber-950">Planter buying-plan review</p>
...
function PlanterBuyingMaterialGroup({ group, compact }: { group: PlanterBoxStockBoardMaterialGroup; compact: boolean }) {
  return (
    <div className={`rounded-md border ${group.reviewReasons.length > 0 ? "border-amber-200 bg-amber-50" : "border-sawdust bg-shop/30"} p-3`}>
      ...
      <div className="mt-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-ink/55">Planter panels from this material</p>
        <PlanterStockBoardPlanningGraphic group={group} />
      ...
      <p className={`mt-3 rounded-md border border-sawdust bg-white p-2 ${compact ? "text-xs" : "text-sm"} leading-6 text-ink/65`}>
        Stock length, finish, liner, and drainage details still need review.
      </p>
    </div>
  );
}

function PlanterStockBoardPlanningGraphic({ group }: { group: PlanterBoxStockBoardMaterialGroup }) {
  const visiblePieces = group.pieces.slice(0, 5);
  const needsReview = group.reviewReasons.length > 0 || group.thickness.status === "missing" || group.pieces.some((piece) => piece.needsReview);
  const visualLabel = `${group.displayName} planter stock-board planning visual: ${group.pieces.map((piece) => piece.printLabel).join(", ")}; stock length to select; not optimized`;
  ...
  return (
    <svg className="mt-3 h-28 w-full rounded-md border border-sawdust bg-white" viewBox="0 0 420 112" role="img" aria-label={visualLabel}>
      ...
      {visiblePieces.map((piece, index) => {
        ...
        return (
          <g key={piece.id}>
            <rect x={x} y="70" width="48" height="24" rx="4" fill={fill} stroke="#7a5b2e" strokeWidth="1.5" strokeDasharray={piece.needsReview ? "4 3" : undefined} />
            <text x={x + 24} y="86" textAnchor="middle" className="fill-ink text-[9px] font-semibold">
              {piece.partLabel}
            </text>
            <text x={x + 24} y="106" textAnchor="middle" className="fill-ink text-[9px] font-semibold">
              {piece.quantityLabel}
            </text>
          </g>
        );
