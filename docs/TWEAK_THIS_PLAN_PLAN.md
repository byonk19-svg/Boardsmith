...
Task 73M/74A hosted verification confirmed the active hosted flow on June 10, 2026 through the authenticated hosted smoke session: route smoke reached `/projects`, one non-critical active project revision saved a new latest plan, the previous plan remained in history, revised-vs-prior copy and the `Revised` history marker appeared, and the revised latest print preview rendered expected shop-plan sections. The archived-project live UI check has a narrow caveat because the hosted Archived filter did not expose an archived project card in that session; rendered-route tests still cover archived revision blocking.

Task 75A closed that hosted archived-project caveat on June 10, 2026. A clearly labeled non-critical smoke/test project with a latest plan was archived during smoke; the Archived filter exposed it, Active excluded it, archived detail stayed accessible, the restore action was visible, `Tweak this plan` showed restore-before-revising copy instead of a revision form, and archived print preview remained accessible.
