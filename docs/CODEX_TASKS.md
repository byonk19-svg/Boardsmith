...
## Task 73E - Secret-Safe Hosted Smoke Bypass Path

- [x] Confirm Vercel documents Protection Bypass for Automation with the `x-vercel-protection-bypass` header and optional `x-vercel-set-bypass-cookie: true` browser-follow-up cookie behavior.
- [x] Add `docs/HOSTED_SMOKE_AUTOMATION.md` covering the one-time Vercel setup, `boardsmith-hosted-smoke` labeling, secret-safe env vars, route-level check, browser smoke header/cookie flow, redaction rules, and rotation/revocation.
- [x] Add `scripts/hosted-smoke-check.mjs` as a no-dependency route helper that reads bypass and access values from env only, sends the Vercel bypass headers, preserves the Boardsmith `/access` gate, and prints sanitized status without hosted URLs, hosts, secrets, cookies, headers, project refs, row data, or sensitive logs.
- [x] Add focused script coverage for bypass headers, access-cookie flow, missing-secret failure, and output redaction.
- [x] Update hosted deployment/readiness docs and `.env.example` with secret-safe variable names only.
...
## Task 73G - Ignored Local Env Support for Hosted Smoke

- [x] Confirm `.env.hosted-smoke.local` is already ignored by the existing `.gitignore` pattern `.env*.local`.
- [x] Add `npm run smoke:hosted` to load `.env.hosted-smoke.local` when present and run `scripts/hosted-smoke-check.mjs`.
- [x] Use Node's `--env-file-if-exists` flag so the helper still prints sanitized `missing_required_env` output when the ignored local file has not been created.
...
## Task 83A - Wall-Shelf Buying Plan Checkpoint

- [x] Document commit `258e2f5 Add wall shelf buying plan view model` in the private readiness checkpoint notes.
- [x] Record the deterministic path: Build Model -> Cut View Model -> Buying Plan View Model -> deterministic renderer.
- [x] Record that Buying Plan now appears between Cut Checklist and Materials on detail and print pages.
- [x] Record the updated shared packet flow: Snapshot -> Visuals -> Cut -> Buying Plan -> Materials -> Build -> Check -> Reference.
- [x] Record that Buying Plan groups Build Model cut pieces by material while keeping stock length as selection/review language.
- [x] Record that no exact stock-board purchase, price, vendor, inventory, CAD, CNC, app-generated PDF, export, cut-optimization, or AI-image behavior was introduced.
- [x] Confirm the invalid five-shelf wall shelf with `0.1 in` total height remains review-needed.
- [x] Confirm single shelf and valid five-shelf wall shelf regressions passed.
- [x] Confirm browser verification included a disposable local valid five-shelf wall shelf with `60 in` total height.
- [x] Confirm validation passed: `npm run lint`, `npm run typecheck`, `npm test`, `npm run build`, and `git diff --check`.
...
## Task 89C - Wall-Shelf Missing Detail Question Model

- [x] Implement [#4 Add missing-detail question model for wall shelves](https://github.com/byonk19-svg/Boardsmith/issues/4).
- [x] Cover concrete questions and reasons for dimensions, material thickness/type, shelf layout/count, connected-unit support/frame, mounting method, expected load/use, tools, child/electrical review, and finish/exposure.
- [x] Keep separate shelves and connected shelf units on different support/layout question paths.
- [x] Keep question copy in the classifier so the Clarification Gate panel can render it without duplicating copy logic.
- [x] Distinguish outdoor/exposure wording from actual finish, fastener, and weather-protection details.
...
## Task 90D - Wall-Shelf Stock-Board Planning Visual

- [x] Implement [#9 Add wall-shelf stock board cut layout visual](https://github.com/byonk19-svg/Boardsmith/issues/9).
- [x] Add a deterministic stock-board planning visual to the wall-shelf Buying Plan material groups.
- [x] Use the same standardized part identifiers and full accessible part labels as the cut list, Buying Plan, materials, build steps, and print.
- [x] Show review-needed states for missing material thickness, unresolved support/frame work, and stock-length selection.
- [x] Preserve conservative copy: no exact purchase, vendor, price, cart, inventory, or cut-optimization claim.
...
## Task 90K - Hosted Private-Use Signal Pass

- [x] Run hosted route/content smoke through the protected hosted path before private-use checks.
- [x] Create and generate three fresh clearly labeled non-critical wall-shelf projects without recording hosted URLs, project IDs, cookies, headers, screenshots, row data, session files, or secrets.
- [x] Create and generate two fresh clearly labeled non-critical planter-box projects without recording hosted URLs, project IDs, cookies, headers, screenshots, row data, session files, or secrets.
- [x] Confirm no generated-plan blocks repeated across the five supported non-critical projects.
- [x] Confirm planter packets preserve Planter Box Buying Plan, drainage/liner review, panel-connection review, Build Guide, and panel/part labels.
