...
The June 20, 2026 deeper planter packet planning follow-through landed locally and passed the focused packet/detail/print tests. A follow-up on the same day added planter-specific readiness actions for drainage, liner, outdoor finish, stock-board selection, panel cuts, and connection review. On June 20, 2026, planter-specific hosted detail and print route/content smoke also passed through the protected hosted path using sanitized `/projects/[id]` and `/projects/[id]/print` output plus shared readiness sentinels for `Plan Readiness / Next Actions`, `Drainage and liner approach needs review`, and `Panel connections need manual review`. On June 21, 2026, a clearly labeled non-critical hosted planter project generated and saved successfully, and generated-plan detail plus Browser Print Plan preserved planter-specific review language without positive export/CAD/CNC/shopping/vendor/price/cart/load-rating/approval claims. A same-day friction pass confirmed two low-risk wall-shelf Buying Plans still repeated stock-length and hardware review gaps, so a narrow wall-shelf Buying Plan follow-through now makes those pre-purchase decisions explicit without choosing exact products or shopping behavior. On June 22, 2026, five fresh clearly labeled non-critical hosted private-use projects generated and rendered successfully across wall-shelf and planter scenarios. After the buying-decision branch was merged and deployed, a fresh hosted wall-shelf buying-trip pass confirmed the deployed detail and Browser Print Plan routes render the explicit `Buying decisions before purchase` checklist. The pass did not expose repeated wall-shelf buying-trip friction, and the only shopping-cart wording was the explicit no-shopping-cart guardrail. Planter connection/build-step confusion did not repeat in the broader private-use signal pass. No hosted URL, secret, cookie, request header, project ID, row data, screenshot, or sensitive log was recorded.
...
- Buying Plan is still conservative. It does not choose exact stock boards, optimize cuts, price materials, choose vendors, or produce a complete shopping list. Fresh hosted dogfood confirmed stock-length selection was the repeated wall-shelf gap; the packet now exposes it as an explicit buying decision instead of selecting a board.
- Hardware and fastener guidance remains review-first because exact bracket/anchor choices depend on wall structure, expected load, and real site review. Fresh hosted dogfood confirmed this was still the main buying-trip handoff to watch; the packet now groups it as an explicit hardware/site review decision without choosing products.
- The fresh hosted planter generated-plan dogfood pass covered one realistic non-critical planter and did not expose a repeated failure mode. More planter work should still wait for repeated private-use friction rather than speculation.
- The June 22 hosted private-use pass generated all five non-critical wall-shelf/planter projects without repeated generation blocks. A post-merge hosted wall-shelf buying-trip pass then validated that the deployed detail and Browser Print Plan routes render the explicit wall-shelf buying-decision checklist.
...
1. Keep using wall-shelf Buying Plans manually and only expand behavior if the explicit buying decisions remain repeatedly annoying in real buying/building use.
