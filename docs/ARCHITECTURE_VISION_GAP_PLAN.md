...
**June 20 follow-through**: planter boxes now have the first bounded non-wall-shelf packet-depth slice: deterministic part identity, review-first cut-layout data, review-first stock/material planning data, a planter-specific hero, and planter-specific readiness actions wired through the printable manifest, detail page, and print page. This intentionally does not add planter-specific build-step, export, shopping, or full diagram families; those remain generic adapters until repeated dogfood friction justifies more template-specific modules. A deletion-test review also found that a broad shared detail/print app-layer packet renderer would be shallow right now, so route-specific summaries and print-table behavior remain intentional while the shared seam stays at `PrintablePlanManifest` and `printable-plan-packet`. A later follow-through extracted the repeated template packet dispatch into `app/projects/[id]/PlanPacketSections.tsx` so detail and Browser Print Plan choose hero visuals, project visuals, readiness, cut diagram, buying plan, and build guide through one app-layer module without flattening the routes into one renderer.

**June 22 follow-through**: planter boxes now have a bounded typed Build Guide from deterministic planter Build Model operations and packet part labels. This closes the generic build-step fallback for rectangular planter boxes while keeping the output review-first. Planter connection diagrams, exact joinery, exact fastener spacing, optimized stock-board/course counts, vendor shopping, exports, and broader template families remain deferred.
